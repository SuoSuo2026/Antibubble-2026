...
def build_context(output_dir, valid_frame_range, roi, tracking_overrides):
    ...
    config["video_export"]["filename"] = f"{CASE_NAME}_group_meeting_monitor.mp4"
    config["video_export"]["frame_range_mode"] = "valid"
    config["video_export"]["crop_to_roi"] = True
    config["video_export"]["scale_bar_y"] = min(220, roi["h"] - 20)
...
def fit_acceleration_oscillation(context, tracking_outputs):
    ...
    plot_acceleration_summary(outdir, fit_results, summary)
    return summary, serializable


def fit_prefusion_quadratic_acceleration(context, tracking_outputs):
    outdir = context["output_dir"] / "acceleration_prefusion_quadratic"
    outdir.mkdir(parents=True, exist_ok=True)

    all_data = results_to_dict(tracking_outputs["results"])
    frame_windows = [
        [149, 164],
        [149, 165],
        [149, 166],
    ...
    for smooth_window in smooth_windows:
        for frame_window in frame_windows:
            valid_data = apply_valid_frame_window(all_data, tuple(frame_window))
            kin = compute_kinematics(
                data=valid_data,
                fps=context["exp_cfg"]["fps"],
                motion_axis=context["exp_cfg"]["motion_axis"],
            ...
            if valid.sum() < 10:
                continue
            t_fit = t[valid] - np.mean(t[valid])
            x_fit = x[valid]
            coef = np.polyfit(t_fit, x_fit, 2)
            curve = np.polyval(coef, t_fit)
            residual = x_fit - curve
            fit_results.append(
                {
                    "smooth_window": smooth_window,
                    "frame_window": frame_window,
                    "a_quad_mm_s2": float(2.0 * coef[0]),
                    "residual_rms_mm": float(np.sqrt(np.mean(residual**2))),
                }
            )

    a_values = np.array([item["a_quad_mm_s2"] for item in fit_results], dtype=float)
    summary = {
        "case": CASE_NAME,
        "method": "quadratic_fit_prefusion_windows",
        "valid_frame_range": context["exp_cfg"]["valid_frame_range"],
        "n_fits": int(len(fit_results)),
        "a_mean_mm_s2": float(np.mean(a_values)),
        "a_std_mm_s2": float(np.std(a_values)),
        "a_rel_std_percent": float(100.0 * np.std(a_values) / abs(np.mean(a_values))),
        "a_min_mm_s2": float(np.min(a_values)),
        "a_max_mm_s2": float(np.max(a_values)),
        "oscillation_frequency_hz": None,
        "note": "The pre-fusion window is too short for a stable quadratic+oscillation frequency scan.",
    }
    with open(outdir / "a_prefusion_quadratic_summary.json", "w", encoding="utf-8") as f:
        json.dump({"summary": summary, "fits": fit_results}, f, indent=2)

    idx = np.arange(1, len(a_values) + 1)
    fig, ax = plt.subplots(figsize=(10, 4), dpi=200)
    ax.plot(idx, a_values, "o-", ms=4, lw=1.8)
    ax.axhline(summary["a_mean_mm_s2"], ls="--", lw=1.8)
    ax.fill_between(
        idx,
        summary["a_mean_mm_s2"] - summary["a_std_mm_s2"],
        summary["a_mean_mm_s2"] + summary["a_std_mm_s2"],
        alpha=0.18,
    )
    ax.set_title(f"{CASE_NAME}: pre-fusion quadratic acceleration")
    ax.set_xlabel("Fit case index")
    ax.set_ylabel("a_quad [mm/s^2]")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(outdir / "a_prefusion_quadratic_summary.png", bbox_inches="tight")
    plt.close(fig)

    return summary, fit_results


def plot_acceleration_summary(outdir, fit_results, summary):
    a_values = np.array([item["a_fit_osc_mm_s2"] for item in fit_results], dtype=float)
    idx = np.arange(1, len(a_values) + 1)
    fig, ax = plt.subplots(figsize=(10, 4), dpi=200)
    ax.plot(idx, a_values, "o-", ms=4, lw=1.8)
    ...
    ax.set_title(f"{CASE_NAME}: pre-fusion a_fit_osc")
    ax.set_xlabel("Fit case index")
...
def make_final_four_panel(context, analysis_outputs, acc_summary, acc_fits):
    ...
    a_values = np.array([item["a_quad_mm_s2"] for item in acc_fits], dtype=float)
    ...
    ax_a.axhline(acc_summary["a_mean_mm_s2"], ls="--", lw=1.8)
    ax_a.fill_between(
        fit_index,
        acc_summary["a_mean_mm_s2"] - acc_summary["a_std_mm_s2"],
        acc_summary["a_mean_mm_s2"] + acc_summary["a_std_mm_s2"],
        alpha=0.18,
    )
    ax_a.set_title("a: pre-fusion quadratic fits")
    ax_a.set_xlabel("Fit case index")
    ax_a.set_ylabel("a_quad [mm/s^2]")
    ax_a.grid(alpha=0.3)
    ...
    ax_note.axis("off")
    text = (
        f"Characteristic values for {CASE_NAME} before fusion\n"
        f"a_quad = {acc_summary['a_mean_mm_s2']:.1f} +/- "
        f"{acc_summary['a_std_mm_s2']:.1f} mm/s^2 "
        f"({acc_summary['a_mean_mm_s2'] / 1000.0:.4f} +/- "
        f"{acc_summary['a_std_mm_s2'] / 1000.0:.4f} m/s^2; "
        f"relative std = {acc_summary['a_rel_std_percent']:.2f}%)    |    "
        f"R = {r_mean:.4f} +/- {r_std:.4f} mm "
        f"(relative std = {r_rel:.2f}%)    |    "
        f"oscillation frequency f = not fitted for this short pre-fusion segment    |    "
    ...
    metrics = {
        ...
        "a_quad_mean_mm_s2": acc_summary["a_mean_mm_s2"],
        "a_quad_std_mm_s2": acc_summary["a_std_mm_s2"],
...
def main():
    output_dir = (
        ...
        / "processed_data"
        / CASE_NAME
        / f"{CASE_NAME}_group_meeting"
    )
    roi = {"x": 0, "y": 150, "w": 1870, "h": 240}
    valid_frame_range = [149, 166]
    tracking_overrides = {
    ...
    acc_summary, acc_fits = fit_prefusion_quadratic_acceleration(context, tracking_outputs)
